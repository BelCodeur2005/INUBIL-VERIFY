...
**The actual wired-up storage path is `backend/src/storage` (S3/R2), not IPFS.** `documents.service.ts` uploads the PDF through `StorageService` and persists the key on `documents.pdf_url`; presigned download URLs also go through `StorageService.getPresignedUrl`. `StorageService` auto-selects Cloudflare R2 if `CLOUDFLARE_ACCOUNT_ID` is set, else plain AWS S3 (via `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`/`AWS_S3_BUCKET`) — none of those vars are in `.env.example` yet, add them when configuring storage.

`backend/src/ipfs` (Pinata, `PINATA_API_KEY`/`PINATA_SECRET_KEY`) exists and has its own spec, but `IpfsService` is not injected anywhere else in `src/` — it's currently dead code, not part of the emission flow. `documents.cid_ipfs` is only read in the response DTO and is never written, so it's effectively always `null` right now. Don't assume IPFS is live in the current build; verify by re-grepping for `IpfsService` usages before relying on this changing. `docs/guide-aws-s3.md`, `docs/guide-cloudflare-r2.md`, and `docs/guide-pinata-ipfs.md` cover setup for each backend if/when this gets reconciled.
