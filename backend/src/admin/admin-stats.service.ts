...
export class AdminStatsService {
  ...
  async statistiquesGlobales(): Promise<StatistiquesGlobalesDto> {
    ...
    const [
      ...
      this.prisma.documents.count({ where: { statut: 'brouillon', deleted_at: null } }),
      ...
      this.prisma.documents.count({ where: { statut: 'rejete', deleted_at: null } }),
      this.prisma.documents.count({ where: { statut: 'expire', deleted_at: null } }),
      this.prisma.documents.count({ where: { transaction_hash: { not: null }, deleted_at: null } }),
    ...
    return {
      ...
      documents: {
        total:             docsTotal,
        brouillons:        docsBrouillons,
        en_validation:     docsEnValidation,
        actifs:            docsActifs,
        revoques:          docsRevoques,
        rejetes:           docsRejetes,
        expires:           docsExpires,
        ancres_blockchain: docsAncres,
      },
