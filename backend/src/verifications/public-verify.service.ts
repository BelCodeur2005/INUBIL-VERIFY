...
} from '../documents/rapport-verification-pdf.service';
...
export class PublicVerifyService {
  ...
  async verifierParIdentifiant(
    ...
    const infoBlockchain = doc
      ? await this.lireBlockchain(doc.numero_unique)
      : null;
    const resultat = this.evaluerStatut(doc, infoBlockchain);
    return this.construireReponse(
      resultat,
      doc,
      null,
      'lien_unique',
  ...
  private messagesPourResultat(
    resultat: VerifyResponseDto['resultat'],
  ): string {
    const messages = {
      authentique:
        'Ce document est authentique et certifié sur la blockchain INUBIL.',
  ...
  private conseilPourResultat(resultat: VerifyResponseDto['resultat']): string {
    if (resultat === 'authentique') {
      return (
        'Comparez les informations affichées avec celles présentes sur le document physique. ' +
  ...
  private formaterDocumentPublic(doc: any): DocumentPublicDto {
    const matieres: MatierePublicDto[] = (doc.matieres_document ?? []).map(
      (m: any) => ({
  ...
  private async construireReponse(
    ...
    const etudiantUtilisateurId = doc?.etudiants?.utilisateur_id;
    if (etudiantUtilisateurId) {
      this.notificationsInApp
        .creer({
          utilisateurId: etudiantUtilisateurId,
        ...
        .catch((err) =>
          this.logger.error(
            `Notification in-app verification echouee pour doc ${doc.id} : ${err.message}`,
          ),
        );
    }
  ...
  private async lireBlockchain(
    numeroUnique: string,
  ): Promise<BlockchainInfoDto | null> {
